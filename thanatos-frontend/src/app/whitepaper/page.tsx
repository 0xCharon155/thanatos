  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-10">
      ...
      <Box>
        ...
        3. Verified-dead tokens (voucher from the verifier) earn 38 karma ×1.5 plus a log-scaled amount bonus, and extend the clock (max +30 min per wallet per epoch).<br />
        4. Unverified tokens earn 5 karma per burn, at most 25 per wallet per epoch, and never move the clock.<br />
        5. When the clock hits zero the epoch seals; anyone can call seal(). A full soul bar shortens the clock, but never below 6 hours from the epoch start.<br />
        6. If the treasury holds less than 0.01 ETH at that moment, the epoch is extended by 6 hours instead, and burning continues.<br />
        7. The keeper stages a name from the ashes and calls rebirth(): the contract splits the treasury and launches on Pons. Every burner claims a share of the seed buy by karma.<br />
      ...
      <Box>
        verified:   karma = (38 + log10(amount + 1)) × mult    mult = 1.5 dead · 1.0 alive<br />
        unverified: karma = 5 per burn, max 25 per wallet per epoch, no clock bonus<br />
        <br />
        Examples (verified dead): 1,000 tokens = 61.5 · 1,000,000 = 66 · 1,000,000,000 = 70.5 karma. One verified burn always outweighs a wallet&apos;s whole unverified cap.
      </Box>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {[
          ...
          ["AIRDROP", "The seed buy of each new token is held by the Reincarnator contract. Every burner claims their share, pro-rata by epoch karma, straight from the contract."],
      ...
      <Box>
        ...
        - The keeper key can call collect(), seal(), stage metadata, rebirth() and runBuyback(). It cannot move ETH or tokens; airdrop shares are computed on-chain.<br />
        - rebirth() can only send ETH to three fixed executors: the founder splitter, the Reincarnator (fixed Pons forwarder) and the Buyback. A buyback that cannot execute keeps its ETH in a reserve; it is never merged into fee share.<br />
        - The Buyback route can only change 48 hours after the change is proposed on-chain.<br />
      ...
      <Box>
        ...
        Epoch 1 clock: 6 hours · later epochs: 24 hours · minimum epoch length 6 hours · minimum treasury to seal 0.01 ETH (frozen after setup)<br />
        ...
        Verified: 38 karma × 1.5 dead / 1.0 alive, plus log10(amount + 1) · Unverified: 5 karma per burn, 25 cap per wallet per epoch<br />
        Split: 20% protocol · then 30% fee share / 40% seed / 30% $THANATOS buyback-burn<br />
        Airdrop: every burner, pro-rata by epoch karma, claimed from the Reincarnator<br />
        No presale - no team allocation - no staking lockups
      </Box>

      <H n="10">RISKS (READ THIS)</H>
      <P>Rebirth tokens are memecoins created by an algorithm; they can go to zero. Fee share depends on trading volume and altar activity that may never materialize; it can be zero. Smart contracts are unaudited. Burned tokens cannot be recovered under any circumstances. The altar fee is not refundable. Only sacrifice what you already consider worthless. The protocol depends on Pons v2: the Pons owner can change a launch&apos;s creator fee recipient behind a 3-day on-chain timelock, and $THANATOS buybacks require a route for wherever the token trades (the bonding curve first, a Uniswap v4 pool after graduation).</P>
