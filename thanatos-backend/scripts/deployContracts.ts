 ...
 *      FOUNDER_A, FOUNDER_B, PONS_FACTORY_ADDRESS, PONS_FORWARDER_ADDRESS, PONS_FEE_ESCROW,
...
async function main() {
  ...
  const escrow = env("PONS_FEE_ESCROW", "0xd3AFEB2a57f70eF218Aa82451c51B2fb0416Ac9e");
  ...
  if (deployer.address.toLowerCase() === agent.toLowerCase()) throw new Error("deployer must not be the agent key");
  ...
  if (network.name !== "hardhat") {
    ...
    if ((await f.feeEscrow()).toLowerCase() !== escrow.toLowerCase()) throw new Error("escrow mismatch");
  ...
  const altar = await ethers.deployContract("ThanatosAltarV2", [deployer.address, agent, verifier, await splitter.getAddress(), escrow, firstEpoch, target]);
  await altar.waitForDeployment();
  const re = await ethers.deployContract("Reincarnator", [await altar.getAddress(), forwarder, factory, pairToken, configId, taxBps]);
  ...
  console.log("\nNEXT: after $THANATOS launches -> bb.proposeRoute(curve), 48h later bb.applyRoute(); after the test epoch -> altar.freeze()");
  ...
  if (network.name !== "hardhat" && process.env.SKIP_VERIFY !== "1") {
    ...
    await verify(out.ALTAR_ADDRESS, [deployer.address, agent, verifier, out.FOUNDER_SPLITTER_ADDRESS, escrow, firstEpoch, target]);
    await verify(out.REINCARNATOR_ADDRESS, [out.ALTAR_ADDRESS, forwarder, factory, pairToken, configId, taxBps]);
